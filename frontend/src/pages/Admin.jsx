...
const Admin = () => {
	...
	const fetchScientist = async () => {
		try {
			const response = await axios.get(`http://localhost:5100/api/scientists/${id}`);
			...
			<div className="panel-bio">
				<form onSubmit={handleSubmit}>
					<div className="form-inner">
						...
						<input placeholder="E.g., Einstein" type="text" name="name" value={formData.name} onChange={handleChange} />
					</div>

					<div className="form-inner">
						<label>Field:</label>
						<input placeholder="E.g., Physics" type="text" name="field" value={formData.field} onChange={handleChange} />
					</div>

					<div className="form-inner">
						<label>Nationality:</label>
						<input placeholder="E.g., German" type="text" name="nationality" value={formData.nationality} onChange={handleChange} />
					</div>
				...
				<div className="biography-add">
					...
					<div className="quill-container">
						<ReactQuill placeholder="Write something about your favorite scientist" style={{ height: "22.5rem" }} theme="snow" value={formData.biography} onChange={handleBiographyChange} />
					</div>
				</div>
			</div>
		</div>
